...
analyseStm :: LocalScope KernelsMem m => LastUseMap -> InUse -> Stm KernelsMem -> m (InUse, LastUsed, Graph VName)
...
analyseExp :: LocalScope KernelsMem m => LastUseMap -> InUse -> Exp KernelsMem -> m (InUse, LastUsed, Graph VName)
...
analyseKernelBody :: LocalScope KernelsMem m => LastUseMap -> InUse -> KernelBody KernelsMem -> m (InUse, LastUsed, Graph VName)
analyseKernelBody lumap inuse body = analyseStms lumap inuse $ kernelBodyStms body

analyseBody :: LocalScope KernelsMem m => LastUseMap -> InUse -> Body KernelsMem -> m (InUse, LastUsed, Graph VName)
analyseBody lumap inuse body = analyseStms lumap inuse $ bodyStms body

analyseStms :: LocalScope KernelsMem m => LastUseMap -> InUse -> Stms KernelsMem -> m (InUse, LastUsed, Graph VName)
...
analyseSegOp :: LocalScope KernelsMem m => LastUseMap -> InUse -> SegOp lvl KernelsMem -> m (InUse, LastUsed, Graph VName)
...
segWithBinOps :: LocalScope KernelsMem m => LastUseMap -> InUse -> [SegBinOp KernelsMem] -> KernelBody KernelsMem -> m (InUse, LastUsed, Graph VName)
segWithBinOps lumap inuse binops body = do
  (inuse', lus', graph) <- analyseKernelBody lumap inuse body
  (inuse'', lus'', graph') <- mconcat <$> mapM (analyseSegBinOp lumap inuse') binops
  return (inuse'', lus' <> lus'', graph <> graph')

analyseSegBinOp :: LocalScope KernelsMem m => LastUseMap -> InUse -> SegBinOp KernelsMem -> m (InUse, LastUsed, Graph VName)
analyseSegBinOp lumap inuse (SegBinOp _ lambda _ _) =
  analyseLambda lumap inuse lambda

analyseHistOp :: LocalScope KernelsMem m => LastUseMap -> InUse -> HistOp KernelsMem -> m (InUse, LastUsed, Graph VName)
analyseHistOp lumap inuse histop =
  analyseLambda lumap inuse (histOp histop)

analyseLambda :: LocalScope KernelsMem m => LastUseMap -> InUse -> Lambda KernelsMem -> m (InUse, LastUsed, Graph VName)
analyseLambda lumap inuse (Lambda _ body _) =
  analyseBody lumap inuse body

analyseKernels :: LocalScope KernelsMem m => LastUseMap -> Stms KernelsMem -> m (InUse, LastUsed, Graph VName)
analyseKernels lumap stms =
  mconcat . toList <$> mapM helper stms
  where
    helper :: LocalScope KernelsMem m => Stm KernelsMem -> m (InUse, LastUsed, Graph VName)
