...
lastUseAction =
  ...
  where
    analyseAndPrint :: Prog KernelsMem -> FutharkM ()
    analyseAndPrint prog = do
      let (lumap :: LastUseMap, _) = analyseProg prog
  ...
  where
    ...
    analyseExp (lumap, used) (Op (Inner (SegOp (SegHist lvl _ binops tps body)))) =
      let (lumap', used') = foldr analyseHistOp (lumap, used) binops
          ...
          nms = (freeIn lvl <> freeIn tps) `namesSubtract` used''
       in (insertNames pat_name nms lumap'', used'' <> nms)
    segOpHelper :: (Foldable t, FreeIn lvl, FreeIn tps) => Map VName VName -> Names -> lvl -> t (SegBinOp (Aliases KernelsMem)) -> tps -> KernelBody (Aliases KernelsMem) -> (LastUse, Used)
    segOpHelper lumap used lvl binops tps body =
...
analyseSegBinOp :: SegBinOp (Aliases KernelsMem) -> (LastUse, Used) -> (LastUse, Used)
...
analyseHistOp :: HistOp (Aliases KernelsMem) -> (LastUse, Used) -> (LastUse, Used)
analyseHistOp (HistOp width race dest neutral shp lambda) (lumap, used) =
  let (lumap', used') = analyseLambda (lumap, used) lambda
      nms = (freeIn width <> freeIn race <> freeIn dest <> freeIn neutral <> freeIn shp) `namesSubtract` used'
   in (lumap', used' <> nms)
